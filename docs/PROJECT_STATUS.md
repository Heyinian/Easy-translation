...
| F-006 | AI 场景翻译 profile，支持预置场景（如通用 / 虚幻引擎开发）和自定义场景，并让 AI 翻译源在每次请求时自动套用当前 profile | 中 |
| F-007 | 接入支持用户自填 API key 的 AI 服务商能力，首期以 OpenAI-compatible 协议为主，测试阶段不限定为 OpenAI，并以低 token 消耗策略执行单轮翻译 | 中 |
| F-008 | 可选的输入法式翻译候选词悬浮窗，在外部输入框翻译回填前展示最多 3 个候选并支持数字键选择、提交或取消 | 中 |

> 注：`F-006` / `F-007` / `F-008` 的方案边界已整理到 `docs/ai_translation_profile_plan.md`。`F-007` 首期按 `BYOK` 处理，不限制测试必须使用 `OpenAI`；同时仍不支持直接复用 ChatGPT / DeepSeek 网页或 App 登录态。`F-008` 默认应关闭，并优先服务于短文本 AI 翻译场景。
