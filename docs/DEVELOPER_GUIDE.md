...
- 源码版可从 app.py 正常启动
- .vscode/tasks.json 中已存在“运行源码版 TranslatorApp”任务
- 默认输入窗口翻译触发方式为 triple_space
- 三击空格窗口期当前默认配置为 1.2 秒
- 如果将输入窗口触发方式改成组合键，组合键模式也可工作
- 翻译结果区域支持候选译文选择
- PyInstaller 可构建 dist/TranslatorApp.exe
...
仓库已有 PyInstaller 配置文件 TranslatorApp.spec，可用于构建 exe。
...
- Google / 百度 / 腾讯 目前通常只返回单结果
- Ollama 可请求多候选译文
- UI 已支持候选列表点击切换当前显示结果
- 外部输入框回填当前仍使用主结果，不会等待手动选择候选
...
当前未设置托盘图标资源，运行时可能看到 QSystemTrayIcon::setVisible: No Icon set 提示。这不是致命错误，但属于可优化项。
...
3. 让候选译文可主动回填到外部输入框
4. 补齐托盘图标资源和状态提示
5. 将翻译线程管理进一步封装，减少 main_window.py 复杂度
6. 为外部输入抓取流程增加失败原因分类
