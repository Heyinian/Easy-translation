...
- 测试隔离：设置 `EASY_TRANSLATION_RUNTIME_DIR` 环境变量后，可将设置文件和密钥文件重定向到临时 runtime 目录
...
测试场景下，如果设置了 `EASY_TRANSLATION_RUNTIME_DIR`，`RUNTIME_DIR`、日志目录和截图目录也会切换到该临时目录。
...
### 5.5 最小 automated smoke baseline

- 本地入口：`powershell -ExecutionPolicy Bypass -File scripts\run_smoke_tests.ps1`
- 当前覆盖：`app.py` 路径注入、`config.py` / `settings_manager.py` 基础加载、runtime 隔离、`TranslatorCore` 的基础无网络行为
- 当前不覆盖：真实 GUI 显示、全局热键、剪贴板端到端、OCR、PyInstaller 打包产物
